"""
Smoke test for multi-group queries — classify_groups should extract multiple groups,
each getting their own search results and rationale.
Requires: Ollama + MCP server both running.

Run from project root:
    python -m scripts.test_multi_group
"""
import asyncio
import json
...
async def run():
    ...
    config = {"configurable": {"thread_id": "multi-group-test-1"}}

    # Two distinct needs in one query
    query = "I need emergency shelter for a single adult and food assistance for my kids in the Mission"
    ...
    async for event in graph.astream_events(
        ...
        if kind == "on_chain_end" and name == "classify_groups":
            ...
            print(f"classify_groups → {len(groups)} group(s):")
            ...
            for gid, services in results.items():
                ...
                for svc in services[:3]:
                    ...
                    print(f"  service_id={svc.get('service_id')} cat_match={svc.get('category_match')} elig_match={svc.get('eligibility_match')} cat={svc.get('sfsg_category_names')}{dist}")
            print()
            ...
            for gid, data in formatted.items():
                print(f"format_results group {gid}:")
                print(f"  service_ids ({len(data.get('service_ids', []))}): {data.get('service_ids')}")
